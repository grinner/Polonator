...
void register_image(short unsigned int *base_img,
  ...
  *y_offset = -10000;
  ...
  for(i=0; i<1000-window_width; i+=1){ /* y rows */
    for(j=0; j<1000-window_width; j+=1){ /* x cols */

      curr_score = 0;

      for(k=0; k<window_width; k+=2){ /* y rows */
	for(l=0; l<window_width; l+=2){ /* x cols */
	  ...
	  curr_score += (abs(*(test_img + ((1000 * test_x_idx)+test_y_idx)) - *(base_img + ((1000 * base_x_idx)+base_y_idx))));
	}
      }
