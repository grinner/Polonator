...
class Rotary_valve:
	...
	def usePort(self, valve_name, valve_port):
		log.debug("---\t-\t-->     use rotary valve %s position %d" % (valve_name, valve_port))
		log.debug("---\t-\t-->     set valve %s to position %d" % (valve_name, valve_port))

		self.mux.setToDevice(valve_name)
		self.set_valve_position(valve_port)

		if(valve_name != 'V4'):
			log.debug("---\t-\t-->     set valve %s to position %d" %('V4', int(valve_name[1])))
...
#--------------------------------------------------------------------------------------#
	...
	def set_valve_position(self, valve_position):
		...
		log.debug("---\t-\t-->     set rotary valve to position %s" % valve_position_string)
