...
from mux import Mux # supports serial port multiplexing to address multiple hardware devices
from syringe_pump import Syringe_pump # interface to syringe pump and associated valve
from rotary_valve import Rotary_valve  # interface to a single rotary valve; the valve in use is specified by the mux state
from temperature_control import Temperature_control  # interface to single temp controller; controller in use is specified by mux state
...
class Biochem(Thread):
	...
	def __init__(self, cycle_name, flowcell, logger=None):
		...
		self.cycle_name = cycle_name  # set cycle-name parameter to specified value (4 chars)
		self.cycle = cycle_name[0:3]  # set cycle parameter to specified value (first 3 chars)
		...
		log.info("%s\t%i\t--> Biochemistry object is constructed: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#------------------------------- Get_config_parameters -------------------------------------
	...
	def get_config_parameters(self):
		...
		log.info("%s\t%i\t--> Retrieve configuration parameters from file: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#------------------------------ V to-V4 cleaning ----------------------------
	...
	def clean_V_to_V4(self, rotary_valve):
		...
		log.info("%s\t%i\t--> Clean V to V4 [%s]" % (self.cycle_name, self.flowcell, self.state))
		...
		log.info("%s\t%i\t--> Draw %i ul Wash 1 up to V4" % (self.cycle_name, self.flowcell, self.V_to_V4, 1))
...
#----------------------------------- Flowcell flushing ---------------------------------
	...
	def flush_flowcell(self, V4_port):
		...
		log.info("%s\t%i\t--> Flush flowcell from port %s: [%s]" % (self.cycle_name, self.flowcell, V4_port, self.state))
		...
		log.info("%s\t%i\t--> Flush flowcells fast (%i ul) and eject to waste" % (self.cycle_name, self.flowcell, self.FC_wash*5))
		self.move_reagent(self.FC_wash*5, 24, from_port, self.empty_speed, 3)  #RCT flush flowcells 4-times and eject to waste
#		self.move_reagent(self.FC_wash*5, 29, from_port, self.empty_speed, 3)  #RCT flush flowcells 4-times and eject to waste

		log.info("%s\t%i\t--> Flush flowcells (%i ul) and eject to waste" % (self.cycle_name, self.flowcell, self.FC_wash*3))
...
#------------------------------- Draw reagent into flowcell ----------------------------
	...
	def draw_into_flowcell(self, draw_port, rotary_valve, rotary_port, reagent_volume):
		...
		log.info("%s\t%i\t--> Draw %i ul of reagent into flowcell %i: [%s]" % (self.cycle_name, self.flowcell, reagent_volume, self.flowcell, self.state))
		...
		if rotary_valve == 'V1':
			...
			self.mux.set_to_rotary_valve4()  # switch communic		self.draw_into_flowcell(9, 'V4', 9, 1000) #move 500ul wash into flowcell (port isoprop_port), followed by Wash 1 (port 9)ation to ten port rotary valve V4

		self.rotary_valve.set_valve_position(draw_port)
		log.info("%s\t%i\t--> Push reagent into flowcell with %i ul of Wash" % (self.cycle_name, self.flowcell, FC_draw-self.gap_volume(reagent_volume) - self.air_gap +0))
	...
	def draw_into_flowcell_bufferchase(self, draw_port, rotary_valve, rotary_port, reagent_volume, bufferport, buffervol):
		...
		log.info("%s\t%i\t--> Draw %i ul of reagent into flowcell w/ buffer chase (port %d, volume %d ul):" % (self.cycle_name, self.flowcell, reagent_volume, bufferport, buffervol))
		...
		if buffervol > move_distance:
			log.info("ERROR: buffer volume %d ul specified, max allowed is %d; will only use %d ul\n" % (buffervol, move_distance, move_distance));
		...
		self.rotary_valve.set_valve_position(draw_port)
		log.info("%s\t%i\t--> Push reagent into flowcell with %i ul of Wash" % (self.cycle_name, self.flowcell, move_distance))
...
#---------------------------- Reagent transfer through syringe -------------------------
	...
	def move_reagent(self, fill_volume, from_speed, from_port, to_speed, to_port):
		...
		location in the fluidic system. All parameters biochem.pyare integers respectively."""
...
#----------------------------- Air gap drawing to valves -------------------------------
	...
	def draw_air_to_valve(self, valve, gap_size=None):
		...
		if gap_size == 0:
			...
			log.info("%s\t%i\t--> Draw air bubble to valve %s COM-port: [%s]" % (self.cycle_name, self.flowcell, valve, self.state))
			if valve == 'V1':
				self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4 
				self.rotary_valve.set_valve_position(1)
				self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
			...
			log.info("%s\t%i\t--> Draw %i ul air gap in front of COM-port" % (self.cycle_name, self.flowcell, gap_size))
...
#----------------------------- Room temperature setting --------------------------------
	...
	def set_to_RT(self):
		...
		log.info("%s\t%i\t--> Set temperature controller to %i C: [%s]" % (self.cycle_name, self.flowcell, self.room_temp, self.state))
...
#------------------------- Steady-state temperature waiting ----------------------------
	...
	def wait_for_SS(self, set_temp, poll_temp, tolerance=None):
		...
		log.info("%s\t%i\t--> Wait for steady-state - poll temperature %i C, set_temp %d: [%s]\n" % (self.cycle_name, self.flowcell, poll_temp, set_temp, self.state))
		...
		if set_temp - poll_temp >= 0:  # if ramping up
			if poll_temp >= tc:
				while abs(poll_temp - tc) > tolerance:
					...
					delta = time.time() - t0 # elapsed time in seconds
					if delta > self.time_limit * 60:
						log.warn("%s\t%i\t --> Time limit %s exceeded -> [current: %0.2f, target: %0.2f] C: [%s]" % (self.cycle_name, self.flowcell, self.time_limit, tc, poll_temp, self.state))
						break
...
#					sys.stdout.write("TIME\t ---\t-\t--> Elapsed time: %i s and current temperature: %0.2f C\n" % (int(delta), tc))
		...
		log.warn("%s\t%i\t--> Time to set steady-state temperature: %0.2f minutes and current temperature: %0.2f C: [%s]\r" % (self.cycle_name, self.flowcell, elapsed, tc, self.state))
...
#------------------------- Incubate and count elapsed time ----------------------------
	...
	def incubate_reagent(self, time_m):
		...
		log.info("%s\t%i\t--> Incubate reagent for %i min: [%s]" % (self.cycle_name, self.flowcell, time_m, self.state))
...
#----------------------------------- Ligase mixing -------------------------------------
	...
	def ligase_mix(self, mix_time):
		...
		log.info("%s\t%i\t--> NOT ACTUALLY DOING ANYTHING Mix reagent in mixing chamber for %i seconds: [%s]" % (self.cycle_name, self.flowcell, mix_time, self.state))
...
#----------------------------- Prime rotary valve 1 chambers ---------------------------
	...
	def prime_rotary_valve1(self):
		...
		log.info("%s\t%i\t--> Prime rotary valve V1 reagent block chambers: [%s]" % (self.cycle_name, self.flowcell, self.state))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
		...
		log.info("%s\t%i\t--> Prime position 1 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		...
		log.info("%s\t%i\t--> Prime position 2 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(3)  # switch rotary valve V1 to port 3
		log.info("%s\t%i\t--> Prime position 3 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(4)  # switch rotary valve V1 to port 4
		log.info("%s\t%i\t--> Prime position 4 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(5)  # switch rotary valve V1 to port 5
		log.info("%s\t%i\t--> Prime position 5 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(6)  # switch rotary valve V1 to port 6
		log.info("%s\t%i\t--> Prime position 6 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(7)  # switch rotary valve V1 to port 7
		log.info("%s\t%i\t--> Prime position 7 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(8)  # switch rotary valve V1 to port 8
		log.info("%s\t%i\t--> Prime position 8 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.well_to_V))
		self.move_reagent(self.well_to_V*10, self.fast_speed, 1, self.empty_speed, 3)

		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		self.rotary_valve.set_valve_position(9) # switch rotary valve V1 to port 10
		log.info("%s\t%i\t--> Draw %i ul Wash 1 up to V1-COM" % (self.cycle_name, self.flowcell, 5 * self.wash1_to_V))
...
#----------------------------- Prime rotary valve 2 chambers ---------------------------
	...
	def prime_rotary_valve2(self):
		...
		log.info("%s\t%i\t--> Prime rotary valve V2 reagent block chambers: [%s]" % (self.cycle_name, self.flowcell, self.state))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
...
#----------------------------- Prime rotary valve 3 chambers ---------------------------
	...
	def prime_rotary_valve3(self):
		...
		log.info("%s\t%i\t--> Prime rotary valve V3 reagent block chambers: [%s]" % (self.cycle_name, self.flowcell, self.state))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
		...
		log.info("%s\t%i\t--> Draw %i ul Wash 1 up to V3-COM" % (self.cycle_name, self.flowcell, 5 * self.wash1_to_V))
		self.mux.set_to_rotary_valve3()  # switch communication to ten port rotary valve V3
...
#----------------------------- Prime rotary valve 4 chambers ---------------------------
	...
	def prime_rotary_valve4(self):
		...
		log.info("%s\t%i\t--> Prime rotary valve V4 reagent block chambers: [%s]" % (self.cycle_name, self.flowcell, self.state))

		log.info("%s\t%i\t--> Prime position 1 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.V_to_V4))
		self.mux.set_to_rotary_valve1()  # switch communication to ten port rotary valve V1
		...
		log.info("%s\t%i\t--> Prime position 2 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.V_to_V4))
		self.mux.set_to_rotary_valve2()  # switch communication to ten port rotary valve V2
		...
		log.info("%s\t%i\t--> Prime position 3 with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.V_to_V4))
		self.mux.set_to_rotary_valve3()  # switch communication to ten port rotary valve V3
		...
		log.info("%s\t%i\t--> Draw %i ul guadinine up to V4-COM" % (self.cycle_name, self.flowcell, 5 * self.guadinine_to_V4))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
		...
		log.info("%s\t%i\t--> Draw %i ul NaOH up to V4-COM" % (self.cycle_name, self.flowcell, 5 * self.NaOH_to_V4))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
		...
		log.info("%s\t%i\t--> Draw %i ul dH2O up to V4-COM" % (self.cycle_name, self.flowcell, 5 * self.dH2O_to_V4))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
		...
		log.info("%s\t%i\t--> Draw %i ul Wash up to V4-COM" % (self.cycle_name, self.flowcell, 5 * self.wash1_to_V))
		self.mux.set_to_rotary_valve4()  # switch communication to ten port rotary valve V4
...
#-------------------------- Prime ligase chamber ------------------------
	...
	def prime_ligase(self):
		...
		log.info("%s\t%i\t--> Prime ligase chamber: [%s]" % (self.cycle_name, self.flowcell, self.state))
		log.info("%s\t%i\t--> Prime ligase chamber with %i ul pre-loaded fluid" % (self.cycle_name, self.flowcell, self.ligase_chamber_volume))
...
#-------------------------------- Reagent block priming --------------------------------
	...
	def prime_reagent_block(self, doV4):
		...
		log.info("%s\t%i\t--> Prime reagent block chambers: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#---------------------------------- Priming flowcells ----------------------------------
	...
	def prime_flowcells(self):
		log.info("%s\t%i\t--> Prime both flowcells: [%s]" % (self.cycle_name, self.flowcell, self.state))
		self.flowcell=0
...
#------------------------------ Fluidic sub-system priming -----------------------------
	...
	def prime_fluidics_system(self):
		...
		log.info("%s\t%i\t--> Prime fluidics system: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#-------------------- Temperature control 1-2 initialization ---------------------------
	...
	def temperature_control_init(self):
		...
		log.info("%s\t%i\t--> Initialize temperature controller 1-2: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#-------------------------- Reagent block initialization -------------------------------
	...
	def reagent_block_init(self):
		...
		log.info("%s\t%i\t--> Initialize reagent block cooler: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#-------------------------- Syringe pump initialization -------------------------------
	...
	def syringe_pump_init(self):
		...
		log.info("%s\t%i\t--> Initialize syringe pump: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#--------------------------- Biochemistry initialization -------------------------------
	...
	def init(self):
		...
		log.info("%s\t%i\t--> Initialize biochemistry sub-system: [%s]" % (self.cycle_name, self.flowcell, self.state))
...
#---------------------------------- Strip_chem sub. ------------------------------------
	...
	def strip_chem(self, isoprop_valve, isoprop_port):
		...
		log.info("%s\t%i\t--> In %s subroutine" % (self.cycle_name, self.flowcell, self.state))
		...
		log.info("%s\t%i\t--> Draw %i ul dH2O into system" % (self.cycle_name, self.flowcell, self.dH2O_volume))
		...
		log.info("%s\t%i\t--> Draw %i ul guadinine into system" % (self.cycle_name, self.flowcell, self.guadinine_volume))
		...
		self.draw_into_flowcell_bufferchase(9, 'V4', 5, 200,7,500) #move 500ul isoprop into flowcell (port isoprop_port), followed by Water 1 (port 9)
		self.draw_into_flowcell(7, 'V4', 7, 1000) #one water wash.

		# NOW, MOVE NaOH INTO FLOWCELL
		log.info("%s\t%i\t--> Draw %i ul NaOH into system" % (self.cycle_name, self.flowcell, self.NaOH_volume))
		...
		self.draw_into_flowcell_bufferchase(9, 'V4', 5, 200,7,500) #move 500ul isoprop into flowcell (port isoprop_port), followed by Water 1 (port 9)
		self.draw_into_flowcell(9, 'V4', 9, 1000) #move 500ul wash into flowcell (port isoprop_port), followed by Wash 1 (port 9)

#		log.info("Flush flowcell w/ Wash 1")
# JSE commented out.  Too many washes slowing down the cycle time.  We are doing a wash at the beginning of the hyb.
...
#		self.flush_flowcell(9)
		...
		log.warn("%s\t%i\t--> Finished checmical strip - duration: %0.2f minutes\n" % (self.cycle_name, self.flowcell, delta))
...
#		self.wait_for_SS(self.hyb_set_temp1, self.hyb_poll_temp1, self.temp_tolerance)  # wait until steady-state temperature is reached
		self.draw_into_flowcell(9, 'V4', 9, 1500) #flushflow cell with Wash 1 (port 9)



		log.info("%s\t%i\t--> Draw %i ul anchor primer  into system from rotary valve" % (self.cycle_name, self.flowcell, self.primer_volume))



		log.info("Move primer into flowcell from valve %s, port %d (%d ul)" % (primer_valve, primer_port, self.primer_volume))
...
#		self.wait_for_SS(self.hyb_set_temp2, self.hyb_poll_temp2, self.temp_tolerance)  # wait until steady-state temperature is reached
		...
		log.warn("%s\t%i\t--> Finished primer hybridization - duration: %0.2f minutes\n" % (self.cycle_name, self.flowcell, delta))
...
#-------------------------------- Lig_stepup_peg sub. ----------------------------------
	...
	def lig_stepup_peg(self, nonamer_valve, nonamer_port):
		...
		log.info("Move nonamer mix (%s,%d), preceded by ligation buffer (%s,%d), into flowcell" % (nonamer_valve, nonamer_port, nonamer_valve, 8))
...
#		self.wait_for_SS(self.lig_set_step1, self.lig_poll_step1, self.temp_tolerance)  # wait until steady-state temperature is reached
		self.draw_into_flowcell_bufferchase(9, nonamer_valve, nonamer_port, self.nonamer_volume,8,300)  #RCT push
...
#		self.wait_for_SS(self.lig_set_step4, self.lig_poll_step4, self.temp_tolerance)  # wait until steady-state temperature is reached
		...
		log.warn("%s\t%i\t--> Finished step-up peg ligation - duration: %0.2f minutes\n" % (self.cycle_name, self.flowcell, delta))
...
#------------------------------------- Run sub. ----------------------------------------
	...
	def run(self):
		...
		if self.cycle[0:2] == 'WL' and self.flowcell == 0:	# if white light image cycle on flowcell 0
			...
			log.info("%s\t%i\t--> Device initialization and Exonuclease I digestion is done: [%s]\n" % (self.cycle_name, self.flowcell, self.state))
...
#--------------------------------- Cycle_ligation sub. ---------------------------------
	...
	def cycle_ligation(self):
		...
		log.info("%s %s\t%i\t--> In %s subroutine" % (self.cycle_name, self.cycle, self.flowcell, self.state))
		self.syringe_pump_init()  # initialize syringe pump
		isoprop_valve = 'V4'
		isoprop_port = 5
...
#
	...
	# Call this function to 'rinse' the flowcell; generally speaking, it pulls a large volume of liquid
	# over the flowcell from rinse_port on rinse_valve.  It first pulls fast_vol microliters at fast_speed
	# (generally pulling a vacuum, so volume is not accurate), then pulls slow_vol microliters at slow_speed
	#
	# added by Greg Porreca, Jan-23-2009
	# modified by Richard Terry Jun-6-2009
	def rinse(self, flowcellnum, rinse_valve='V4', rinse_port=9, fast_vol=4000, slow_vol=2000, fast_speed=25, slow_speed=29):
#	def rinse(self, flowcellnum, rinse_valve='V4', rinse_port=9, fast_vol=2000, slow_vol=2000, fast_speed=30, slow_speed=30):
		self.empty_speed = 0
		self.rotary_valve.usePort(rinse_valve, rinse_port)
		self.syringe_pump.draw(fast_vol, flowcellnum, fast_speed, self.empty_speed)
		self.syringe_pump.draw(slow_vol, flowcellnum, slow_speed, self.empty_speed)


	# Call this function to 'pump' reagent into the system from a specific port on a specific valve
	# Note this just pulls the reagent into the system, and doesn't know anything about where the reagent
	# ends up (i.e in the tubing, in the flowcell, etc).  One should understand the path length between
	...
	def waitForTemp(self, setpoint, timeout, flowcellnum, tolerance=None):
		...
		log.debug(" waiting maximum of %d seconds until temperature is within %d C of %d C setpoint" %(timeout, tolerance, setpoint))
		...
		while( (math.fabs(self.temperature_control.get_temperature() - setpoint) > tolerance) and (math.fabs(time.time() - t0)<timeout)):
			sys.stderr.write("\tcurrent temperature: %03d C, elapsed time %04d seconds\r" % (self.temperature_control.get_temperature(), math.fabs(time.time()-t0)))
			sys.stderr.flush()
			time.sleep(1)
		sys.stderr.write("\n")
	...
	def primeReagentBlock(self, flowcellnum, doV4=0):
		...
		for valve in range(1,maxValveRange): 
			for port in range(9,0,-1): # do ports 1-9 in reversr order; port 10 should be plugged, so don't prime this
				log.info('Prime valve %d, port %s with %d ul' %(valve, port, prime_volume[valve]))
				self.rotary_valve.usePort('V' + str(valve), port)
				if((valve == 4) and (port == 8)): # don't prime plugged ports on V4
					log.info('Skip current position because it is plugged')
				else:
					self.syringe_pump.draw(prime_volume[valve], flowcellnum, prime_speed, 0)
	...
	def runILMNDeblock(self, flowcellnum, deblock_valve='V2'):
		...
		self.ilmnDeblock(flowcellnum, deblock_valve, CB_port, deblock_valve, CM_port, deblock_valve, IB_port, deblock_valve, HS_port, deblock_valve, SB_port)
	...
	def runILMNCycle(self, flowcellnum, cycle_valve='V1'):
		...
		self.ilmnCycle(flowcellnum, cycle_valve, IB_port, cycle_valve, IM_port, cycle_valve, HS_port, cycle_valve, SB_port)
	...
	# Perform Illumina's standard 'deblock' protocol using default reaction conditions
	#
	# added by Greg Porreca, Jan-23-2009
	#	
	def ilmnDeblock(self, flowcellnum, CB_valve, CB_port, CM_valve, CM_port, IB_valve, IB_port, HS_valve, HS_port, SB_valve, SB_port):
		log.info("Executing ilmnDeblock %d CB<%s,%d> CM<%s,%d> IB<%s,%d> HS<%s,%d> SB<%s,%d>" %(flowcellnum,
													CB_valve, CB_port,
													CM_valve, CM_port,
													IB_valve, IB_port,
													HS_valve, HS_port,
		...
		volume = 1000
		log.info(" pump %d ul of cleavage buffer from <%s,%d>" %(volume, CB_valve, CB_port))
		self.pumpReagent(flowcellnum, volume, CB_valve, CB_port)

		# now wait for temp to reach setpoint
		timeout=300
		log.info(" wait up to %d seconds for temp to reach %dC setpoint" %(timeout, set_temp))
		self.waitForTemp(set_temp, timeout, flowcellnum)

		# pump 800ul of cleavage mix (CM), then wait 4 minutes
		wait_time=240
		volume=800
		log.info(" pump %d ul of cleavage mix from <%s,%d> and incubate for %d seconds" %(volume, CM_valve, CM_port, wait_time))
		...
		# pump 200ul of cleavage buffer (CB), then wait 4 minutes; incubation is still with CM because of fluid line volume
		wait_time=240
		volume=200
		log.info(" pump %d ul of cleavage buffer from <%s,%d> and incubate for %d seconds (incubation is still with CM)" %(volume, CB_valve, CB_port, wait_time))
		self.pumpReagent(flowcellnum, volume, CB_valve, CB_port)
		time.sleep(wait_time)
		...
		volume = 600
		log.info(" pump %d ul of incorporation buffer from <%s,%d>" %(volume, IB_valve, IB_port))
		self.pumpReagent(flowcellnum, volume, IB_valve, IB_port)

		# pump 600ul high salt (HS) buffer
		volume = 600
		log.info(" pump %d ul of high-salt buffer from <%s,%d>" %(volume, HS_valve, HS_port))
		self.pumpReagent(flowcellnum, volume, HS_valve, HS_port)

		# rinse flowcell thoroughly with Wash 1
		num_rinses = 3
		log.info(" rinse %d times with Wash 1 from valve %s" %(num_rinses, CM_valve))
		...
		volume = 800
		log.info(" pump %d ul of scan buffer from <%s,%d>" %(volume, SB_valve, SB_port))
	...
	# added by Greg Porreca, Jan-23-2009
	#	
	def ilmnCycle(self, flowcellnum, IB_valve, IB_port, IM_valve, IM_port, HS_valve, HS_port, SB_valve, SB_port):
		log.info("Executing ilmnCycle %d IB<%s,%d> IM<%s,%d> HS<%s,%d> SB<%s,%d>" %(flowcellnum,
											    IB_valve, IB_port,
											    IM_valve, IM_port,
											    HS_valve, HS_port,
											    SB_valve, SB_port))
		...
		wait_time=240
		volume=800
		log.info(" pump %d ul of incorporation mix from <%s,%d> and incubate for %d seconds" %(volume, IM_valve, IM_port, wait_time))
		...
		# pump 200ul incorporation buffer (IB), wait for 4 minutes (this moves IM into flowcell)
		wait_time=240
		volume=200
		log.info(" pump %d ul of incorporation buffer from <%s,%d> and incubate for %d seconds (incubation is still with IM)" %(volume, IB_valve, IB_port, wait_time))
		self.pumpReagent(flowcellnum, volume, IB_valve, IB_port)
		time.sleep(wait_time)
		...
		# rinse flowcell thoroughly with Wash 1
		num_rinses = 3
		log.info(" rinse %d times with Wash 1 from valve %s" %(num_rinses, IM_valve))
		for i in range(1,num_rinses+1):
			log.info(" - perform rinse %d / %d" %(i, num_rinses))
	...
	def ilmnHyb(self, flowcellnum, HYB_valve, HYB_port, WASH_valve, WASH_port):
		...
		set_temp=50
		timeout=300
		log.info(" set temperature to %dC, waiting for up to %d seconds" %(set_temp, timeout))
		self.setTemp(flowcellnum, set_temp)
		self.waitForTemp(set_temp, timeout, flowcellnum)
		...
		volume = 800
		wait_time = 60
		log.info(" pump %d ul of hyb mix from <%s,%d>, then incubate for %d seconds" %(volume, HYB_valve, HYB_port, wait_time))
		...
		volume = 200
		wait_time = 60
		log.info(" pump %d ul of wash buffer from <%s,%d> (primer still in flowcell), then incubate for %d seconds" %(volume, WASH_valve, WASH_port, wait_time))
		...
		set_temp=25
		log.info(" set temperature to %dC, and allow to ramp during next step(s)" %(set_temp))
		self.setTemp(flowcellnum, set_temp)

		# rinse flowcell thoroughly with Wash 1
		num_rinses = 3
		log.info(" rinse %d times with Wash 1 from valve %s" %(num_rinses, HYB_valve))
		for i in range(1,num_rinses+1):
			log.info(" - perform rinse %d / %d" %(i, num_rinses))
		...
		log.info(" waiting for up to %d seconds for temperature to reach %d" %(timeout, set_temp))
		self.waitForTemp(set_temp, timeout, flowcellnum)
