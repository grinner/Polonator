...
if len(sys.argv) < 3:
	...
	# Initialize biochemistry object - cycle-name and flowcell-number need to be set later
	biochem = Biochem('WL1', int(sys.argv[1]))  
	method = sys.argv[2]
	log.info("---\t-\t--> Started %s method execution - biochem_utils.py" %(method))
	...
	# usage: biochem_utils.py fcnum draw_into_flowcell
		...
		reagent = "Wash 1"
                biochem.draw_into_flowcell(9, sys.argv[3], int(sys.argv[4]), int(sys.argv[5])) 


	# usage: biochem_utils.py fcnum draw_into_flowcell_bufferchase
	elif method == 'draw_into_flowcell_bufferchase':
		reagent = "Wash 1"
                biochem.draw_into_flowcell_bufferchase(9, sys.argv[3], int(sys.argv[4]), int(sys.argv[5]), int(sys.argv[6]), int(sys.argv[7]) ) 
	...
	# usage: biochem_utils.py fcnum react reagentValve reagentPort temp time bufferBeforeFlag bufferAfterFlag
	# OR biochem_utils.py fcnum react reagentValve reagentPort temp time bufferBeforeFlag bufferAfterFlag bufferPort bufferVolume
	# depending on whether a reaction buffer is used before/after the reagent to prime/flush the lines
	elif method == 'react':
		if(len(sys.argv) == 9):
			biochem.react(sys.argv[3], int(sys.argv[4]), int(sys.argv[5]), int(sys.argv[6]), int(sys.argv[7]), int(sys.argv[8]))
		else:
			biochem.react(sys.argv[3], int(sys.argv[4]), int(sys.argv[5]), int(sys.argv[6]), int(sys.argv[7]), int(sys.argv[8]), int(sys.argv[9]), int(sys.argv[10]))
