...
#		self.move_reagent(self.FC_wash*5, 30, from_port, self.empty_speed, 3)  #RCT flush flowcells 4-times and eject to waste
...
#		commands.getstatusoutput('mplayer -ao alsa speech/strip_chem.wav')
		...
		self.draw_into_flowcell(9, 'V4', 5, 750) #move 750ul isoprop into flowcell (port isoprop_port), followed by Wash 1 (port 9)
...
##################                        BEGIN 'CUSTOMIZABLE' CODE                   ########################
	...
	def runCustomExperiment(self):
		...
		self.react('V1', 2, 55, 30, 0, 0)	
		self.react('V1', 3, 55, 20, 0, 0)
		self.react('V1', 1, 55, 12, 0, 0)	
...
#
	...
	#
	def rinse(self, flowcellnum, rinse_valve='V4', rinse_port=9, fast_vol=4000, slow_vol=2000, fast_speed=13, slow_speed=27):
	...
	def primeReagentBlock(self, flowcellnum, doV4=0):
		...
		for valve in range(1,maxValveRange): 
			for port in range(9,0,-1): # do ports 1-9 in reversr order; port 10 should be plugged, so don't prime this
				...
				if((valve == 4) and ((port == 5) or (port == 8))): # don't prime plugged ports on V4
