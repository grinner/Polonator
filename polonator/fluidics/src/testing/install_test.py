...
print '\n'
# Installation test start.
logger.info('***\t*\t--> Installation testing started - install_test.py')
...
for i in range(0, 2):           # Switch all discrete valves off and on twice.
...
# Switch all rotary valves from port position 1 to 10 in a step-wise manner.
for i in range(1, 11):
...
# Switch all rotary valves from port position 1 to 10 in a step-wise manner.
for i in range(0, 2):
    # Set syringe speed to pull speed (moderately fast).                           
    syringe_pump.set_speed(13)
    syringe_pump.set_absolute_volume(1000)  # Draw 1000 ul of fluid into syringe.
    # Set syringe speed to eject speed (fastest possible on 0-40 scale).
    syringe_pump.set_speed(0)                   
...
def wait_for_SS(target):
    """
    Waits until steady-state temperature is reached, or exits wait block if 
    ramping time exceeds timeout parameter set in manually here.
    """
    ...
    while abs(target - tc) > 1:
        ...
        sys.stdout.write('TIME---\t-\t---> Elapsed time: ' + str(delta) + \
                            ' second \n')
        sys.stdout.flush()
                         
        if delta > time_limit * 60:
            print "--> Time limit %s exceeded -> [current: %f, target: %f] C" \
                    % (time_limit, tc, target)
            break
...
# Set register 0 to 'temperature', a float.
temperature_control.set_temperature(target)
