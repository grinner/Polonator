...
if len(sys.argv) == 1:
    ...
    if method == 'set_to_temperature_control1':
        ...
        # use stdin explicitly and remove trailing newline character
        set_temp = int(sys.stdin.readline().strip())
    ...
    #---------------------------------------------------------------------------
        ...
        print "INFO\t---\t-\t--> Please, enter valve position (1-9) to " + \ 
            "be set [integer]: ",
        # use stdin explicitly and remove trailing newline character
    ...
    #---------------------------------------------------------------------------
        ...
        print "INFO\t---\t-\t--> Please, enter absolute syringe volume " + \
            "(0-1000) to be set [integer]: ",
        # use stdin explicitly and remove trailing newline character
        volume = int(sys.stdin.readline().strip())
        biochem.syringe_pump.set_absolute_volume(volume)

    elif method == 'set_speed':
        print "INFO\t---\t-\t--> Please, enter syringe speed (0-40) to " + \
            "be set [integer]: ",
        # use stdin explicitly and remove trailing newline character
