...
def grep(pattern, f):
    """
    Given a list of files or standard input to read, grep searches for lines 
    of text that match one or many regular expressions, and outputs only the 
    matching lines.
    """
...
def parser(pattern, f=None):
    """
    Retrieves the last log-file modified or opens a specified file, then 
    parses it for given pattern, and finally generates an archive of pattren 
    containing line-logs.
    """
    # pattern log parsing start.
    print '\n--> Pattern-log file generation started'
