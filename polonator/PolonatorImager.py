...
PolonatorImager.py: main imaging thread executed during each sequencing cycle; 
performs either 'WL', or darkfield, imaging (averaging 1 or more images per 
position), or fluorescent imaging in 4 colors.  Sets variables on controller, 
performs autoexposure if necessary, then executes Polonator-acquirer.c, which 
collects the images from the camera and transmits them to the processing 
computer.
...
class Imager(threading.Thread):
    ...
    def __init__(self, cycle, fc):
        ...
        if(self.cycle_name.find('WL') is not -1):
            ...
            self.cycle_name = '%c%c%c%c%c%c' % \
                                    (fn[0], fn[1], fn[2], fn[3], fn[6], fn[7])
        # end if
        else:
    ...
    def run(self):
        ...
        if(self.fluor_imaging == 1):
            ...
            while i < 4:
                ...
                cmd = (os.environ["POLONATOR_PATH"] + \
                            '/bin/Polonator-stagealign %d') % (self.flowcell)
                print cmd
                os.system(cmd)
                ...
                if self.auto_exposure == 1:
                    ...
                    cmd = os.environ["POLONATOR_PATH"] + \
                                    '/bin/PolonatorUtils gotostagealignpos 0 0'
                    print cmd
                    os.system(cmd)

                    cmd = os.environ["POLONATOR_PATH"] + \
                                '/bin/PolonatorUtils snap1 %s %f %d %d' % \
                                (curr_name_new,0.035,j, j+150)
                    print cmd
                    os.system(cmd)
                ...
                self.maestro.setup_imaging(imaging_order[i], \
                ...
                tcmd = (os.environ["POLONATOR_PATH"] + \
                        '/bin/Polonator-acquirer %s %d %d %d %d') % \
                        (curr_name, 
                        self.integration_times[fluors.index(imaging_order[i])]-5, \
        ...
        # base image
            ...
            if(self.dual_fc == 1):
                cmd = (os.environ["POLONATOR_PATH"] + \
                        '/bin/Polonator-stagealign %d 1') % (self.flowcell-2)
            else:
                cmd = (os.environ["POLONATOR_PATH"] + \
                            '/bin/Polonator-stagealign %d 1') % (self.flowcell)
            ...
            self.maestro.setup_imaging('none', self.WL_integration_time-5, \
                                        self.WL_num_to_avg, self.num_arrays, \
                                        self.flowcell, 1, \
                                        self.TDI_flag)
            ...
            """
            cmd = (os.environ["POLONATOR_PATH"] + \
                '/bin/Polonator-acquirer %s %d %d %d %d %d') % \
