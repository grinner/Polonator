 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_display_color_raw.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* display_color_raw.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* display_color_raw.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* display_color_raw (application created by MATLAB Compiler)
		* run_display_color_raw.sh (shell script that can be run 
	...
	All Platforms:            
		* display_color_raw.so (shared library, file extension 
		  varies by platform)
		* display_color_raw.h (library header file)
		* display_color_raw.ctf (Component Technology File archive,
	...
	* display_color_raw.xml (documentation files)
	* display_color_raw.pdb (program database file - if 
	  DEBUG option selected)
	* display_color_raw.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* display_color_raw.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* display_color_raw_1_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* display_color_raw.jar (Java class files and compiled M-code)
	...
	* display_color_raw.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* display_color_raw.xla (Optionally created and placed in
