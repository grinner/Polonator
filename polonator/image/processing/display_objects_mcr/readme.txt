 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_display_objects.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* display_objects.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* display_objects.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* display_objects (application created by MATLAB Compiler)
		* run_display_objects.sh (shell script that can be run 
	...
	All Platforms:            
		* display_objects.so (shared library, file extension 
		  varies by platform)
		* display_objects.h (library header file)
		* display_objects.ctf (Component Technology File archive,
	...
	* display_objects.xml (documentation files)
	* display_objects.pdb (program database file - if 
	  DEBUG option selected)
	* display_objects.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* display_objects.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* display_objects_1_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* display_objects.jar (Java class files and compiled M-code)
	...
	* display_objects.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* display_objects.xla (Optionally created and placed in
