 	...
 	B. Set the path by doing one of the following:
		...
		* Add the MCR directory to the environment variable by issuing 
         ...
         NOTE: When deploying C and C++ standalone applications, it is possible
               to run the shell script file run_disp_regQC.sh on UNIX and Mac
...
3. Collect the following files, based on what you are deploying: 
	...
	All Platforms:
		* disp_regQC.ctf (Component Technology File archive,
		  platform-dependent on target machine)
			
        Windows:
		* MCRInstaller.exe (self-extracting MATLAB Component Runtime
		  library installer)   
		* disp_regQC.exe (application created by MATLAB Compiler)
        UNIX:
		* MCRInstaller.bin (MATLAB Component Runtime library 
	 	  installer)
		* disp_regQC (application created by MATLAB Compiler)
		* run_disp_regQC.sh (shell script that can be run 
	...
	All Platforms:            
		* disp_regQC.so (shared library, file extension 
		  varies by platform)
		* disp_regQC.h (library header file)
		* disp_regQC.ctf (Component Technology File archive,
	...
	* disp_regQC.xml (documentation files)
	* disp_regQC.pdb (program database file - if 
	  DEBUG option selected)
	* disp_regQC.dll (component assembly file)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  
	...
	* disp_regQC.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* disp_regQC_1_0.dll (COM component)
	* MCRInstaller.exe (self-extracting MATLAB Component Runtime
	  library archive)  	
	...
	* disp_regQC.jar (Java class files and compiled M-code)
	...
	* disp_regQC.ctf (Component Technology File archive, 
	  contains compiled M-code)
	* disp_regQC.xla (Optionally created and placed in
