...
#define MAX_OBJSIZE 50 //maximum size of an object in pixels; objects larger than this will not be reported


// GLOBALS
multimap <int, int> m1; // STL multimaps elements can have the same key
map <int, int> m2; // STL map for use in doing sparse arrays, elements have unique key
        ...
        unsigned short *segmask_image)
{
    ...
    /*
    img_mean = stdev(len_w, len_h,raw_image, 1);
    img_stdev = stdev(len_w, len_h,raw_image, 0);
    ...
    */
    ...
    //
    img_mean = mean(len_w, len_h,raw_image);
    img_stdev = stdev2(len_w, len_h,raw_image);
    ...
    fprintf(stdout, "find_objects: mean %d, std2 %d, threshold %d\n", img_mean, img_stdev, CUTOFF);
    ...
    for(count = 0; count < (len_h * len_w); count++)
    { // 1-D index into image
        ...
        //BEADS ARE DARK
        if ((IP[count] < CUTOFF)&&( img_stdev > 100)) 
        { // current pixel is below threshold	

            flag1 = 0; flag2 = 0;
            N1 = count - len_w;
            N2 = count - 1;
            if ((N1 >= 0) && (IP2[N1] > 0) && (IP[N1] < CUTOFF)) {flag1 = 1;} 
            if ((count % len_w != 0) && (IP2[N2]) > 0 && (IP[N2] < CUTOFF)) {flag2 = 1;}
            ...
            {                                                          // connected to both neighbors
	            IP2[count] = IP2[N1];
	            if (IP2[N1] != IP2[N2]) 
	            {                                     // add entry (IP2[N1]=IP2[N2]) to equivalency table 
    ...
    for(count = 0; count < (len_h * len_w); count++)
    {
        if (IP2[count]>0) 
        {
            ...
            *(centroid_xcol + IP2[count]) += (count % len_w) + 1;
            *(centroid_yrow + IP2[count]) += (int) floor(float(count)/len_w) + 1;
        }
    ...
    for(i = 1; i < num_objs; i++)
    {
        ...
        *(obj_xcol + return_index) = (int)floor(*(centroid_xcol + i) / *(obj_size + i)); // assign return args
        *(obj_yrow + return_index) = (int)floor(*(centroid_yrow + i) / *(obj_size + i));
...
{
    multimap<int,int>::iterator iter; // these are iterator objects of a multimap
