...
def goto_flagged_images(self):
    """
        ...
        3) If the image number read differs from the previous image number read in the filestream goto the new image
            by commanding the Maestro motorcontroller to do so
        ...
        6) When you move to a new lane, do a coarse alignment with "Polonator-stagealign"
        7) add the offset to the bead points from 2
        8) when you've read all the beads in an image according to the basecall file, illuminate that image with 
          a image mask on the DMD generated by mapping to the DMD from the camera coordinates
        ...
        the number of beads flagged per lane in a given basecall file is in the *._ file
    ...
    the_files = glob.glob(base_dir + '*.basecalls') # there is a basecall file for each lane?
    ...
    while True:
        ...
        if (the_base_call.isEOF()):   # breaks the loop when a line length is zero indicating EOF
            #print 'breaking'
            ...
            if flag:
                ...
                if last_image != image_number:
                    ...
                    # end if
                    test_case = 0                               # initialize variable so we can loop to find an image header in the object table
                    while (test_case != -1):                    # burn through object table to match base call   
                        raw = theOT.read(4)
                        if len(raw) == 0:                       # if the read returned zero bytes we reached EOF
                            print "Error: ReleaseBeads.py: reached end of file too early likely!"
                            break
                        #end if
                        else :
                    ...
                    #########################
                    # This section synchronizes base call location with object table location
                  
                    last_bead = 0                                   # reset last_bead counter b/c we don't care once we are out of the first seek
                    # Match Flow_cell
                    if the_OT.get_flowcell() < flowcell:
                         the_OT.skip_to_next_flowcell()
                         the_OT.read_entry()                        # important to read in the entry of interest!
                    elif the_OT.get_flowcell() > flowcell:
                        print('Error: somehow flow cell order mismatch between base call and object table')
                    else:
                        pass
                    # end else
                    ...
                    # end if
                        ...
                        print('Error: somehow passing basecall files out of order')
                    # end elif
                    else:
                        pass
                    # end else
                    ...
                    # end if
                         ...
                         print('Error: somehow basecall file has images out of order with object table')
                    # end elif
                    else:
                        pass
                    # end else
                    ...
                    # now do the move
                    if last_cell != flowcell:       # move to new flowcell and align if necessary
                        ...
                        last_cell = flowcell                            
                    #end if
                    MF.goto_image(flowcell,lane,image_number)       # move to new image
                ...
                last_bead = bead                # make sure we are in the right spot in the object table
                # end if
                print 'reading object table'
                the_OT.read_point()             # load point data into memory and advance pointer
                ...
                # end elif
                    ...
                    #MapFunc.add_point_to_table(x_point,y_point, num_beads, coord_list)
